    ...
    def __init__(self, root):
        ...
        def insert():
            ...
            if (len(pId.get()) != 0):
                ...
                # after inserting the data or record to database table
            else:
        ...
        def update():
            ...
            if len(pId.get()) != 0:
                ...
                productList.delete(0, END)
                productList.insert(END,
                                   (pId.get(), pName.get(), pQty.get(), pPrice.get(), pCompany.get(), pContact.get()))
        ...
        '''Create the Frame'''
        MainFrame = Frame(self.root,  bd=50, bg="red")
        ...
        self.ITitle = Label(HeadFrame, font=('arial', 50, 'bold'), fg='red',
                            text='  Warehouse Inventory Sales Purchase  ', bg='black')
        self.ITitle.grid()

        OperationFrame = Frame(MainFrame, bd=1, width=1300, height=60, padx=143, pady=20, bg='white', relief=RIDGE)
        OperationFrame.pack(side=BOTTOM)

        BodyFrame = Frame(MainFrame, bd=2, width=1300, height=400, padx=30, pady=30, bg='white', relief=RIDGE)
        BodyFrame.pack(side=BOTTOM)

        LeftBodyFrame = LabelFrame(BodyFrame, bd=2, width=600, height=380, padx=20, pady=10, bg='yellow', relief=RIDGE,
                                   font=('arial', 15, 'bold'), text='Product Item Details:')
        LeftBodyFrame.pack(side=LEFT)

        RightBodyFrame = LabelFrame(BodyFrame, bd=2, width=400, height=380, padx=20, pady=10, bg='yellow', relief=RIDGE,
                                    font=('arial', 15, 'bold'), text='Product Item Information:')
        RightBodyFrame.pack(side=RIGHT)

        ''' Add the WIidgets to LeftBodyFrame'''

        self.labelpID = Label(LeftBodyFrame, font=('arial', 15, 'bold'), text="Product Id : ", padx=2, pady=2,
                              bg='white', fg='blue')
        ...
        self.labelpName = Label(LeftBodyFrame, font=('arial', 15, 'bold'), text="Product Name : ", padx=2, pady=2,
                                bg='white', fg='blue')
        ...
        self.labelpPrice = Label(LeftBodyFrame, font=('arial', 15, 'bold'), text="Product Price : ", padx=2, pady=2,
                                 bg='white', fg='blue')
        ...
        self.labelpQty = Label(LeftBodyFrame, font=('arial', 15, 'bold'), text="Product Quantity : ", padx=2, pady=2,
                               bg='white', fg='blue')
        ...
        self.labelpCompany = Label(LeftBodyFrame, font=('arial', 15, 'bold'), text="Mfg. Company :  ", padx=2, pady=2,
                                   bg='white', fg='blue')
        ...
        self.labelpContact = Label(LeftBodyFrame, font=('arial', 15, 'bold'), text="Company Contact : ", padx=2, pady=2,
                                   bg='white', fg='blue')
        ...
        self.labelpC6.grid(row=10, column=0, sticky=W)
        ...
        self.buttonSaveData = Button(OperationFrame, text='Save', font=('arial', 18, 'bold'), height=1, width='10',
                                     bd=4, command=insert)
        ...
        self.buttonUpdate = Button(OperationFrame, text='Update', font=('arial', 18, 'bold'), height=1, width='10',
                                   bd=4, command=update)
...
class Database:
    def conn(self):
        ...
        cur = con.cursor()
        query = "create table if not exists product (paid integer primary key,pname text,price text,qty text," \
    ...
    def delete(self, pid="", name="", price="", qty="", company="", contact=""):
        print("Database : delete method called ", pid)
        con = sqlite3.connect("inventory.db")
        cur = con.cursor()
        cur.execute("delete from product where pid=? or name=? or  price=? or qty=? or company=? or contact=?",
